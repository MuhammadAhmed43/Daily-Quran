...
- **Ask (chat) tab `(tabs)/chat.tsx`** — frames 15,16,21,22,23: VOTD banner + Explore-Topics grid + the floating **glass Ask pill**; conversation (curated opener, per-answer toolbar — streaming + image button already exist, restyle); history; topic→suggested-questions sheet.
- **Qur'an reader `(tabs)/quran.tsx`(new) + `surah/[number]`** — frame 36: surah index + reader (pericope subtitle, numbered verses, bottom Surah·Ayah·translation·reciter pager); verse-tap → tafsir/Ask.
...
- **Remaining `components/ui/`:** GlassPill, MediaCard, SectionHeader, Chip, SettingsCard, StatCard, BottomSheet (@gorhom/bottom-sheet NOT installed), SkeletonShimmer, AnalyzingLoader, Primary/SecondaryButton.
...
**GIT:** two commits on `main` atop pushed `8d9f156`, **NOT pushed yet** — `0d82827` (Today redesign + theme/18 palettes + UI kit + 5-tab shell + this spec) and `f956457` (Reflection sheet + prayer time-gate + Today polish + onyx lock). Commit/push ONLY when asked (ASCII msgs via `git commit -F`, no Co-Authored-By, quote bracket paths). `Bible-chat/` + `Ui-Design/` are gitignored.

**NEXT STEP:** redesign the next tab — **Ask** or **Qur'an** (user picks) — in onyx, device-reviewing like Today. Reuse the accordion recipe + UI kit + theme tokens; preserve ALL existing logic (re-skin only).
