...
- **Ask (chat) tab `(tabs)/chat.tsx`** — frames 15,16,21,22,23: VOTD banner + Explore-Topics grid + the floating **glass Ask pill** (reuse `glass-surface.tsx`); conversation (curated opener, per-answer toolbar — streaming + image button already exist, restyle); history; topic→suggested-questions sheet. **← NEXT.**
...
- **Remaining `components/ui/`:** GlassPill (the Ask input bar — but **`glass-surface.tsx` is DONE**: an adaptive Liquid-Glass/minimal capsule; reuse it), MediaCard, SectionHeader, Chip, SettingsCard, StatCard, BottomSheet (@gorhom/bottom-sheet NOT installed — the **reflect-style RN-`Modal` slide-up sheet** is the established pattern; reuse it), SkeletonShimmer, AnalyzingLoader, Primary/SecondaryButton.
...
**GIT:** `main` is PUSHED + in sync with `origin/main` at `b75e7ba`. Redesign chain (all pushed): `0d82827` (Today + theme/18 palettes + UI kit + 5-tab shell + spec) → `f956457` (Reflection sheet + prayer gate + Today polish + onyx lock) → `9d12504` (spec) → `b75e7ba` (Qur'an tab: reader + index + all sheets + `glass-surface` + `expo-glass-effect` dep). Commit/push ONLY when asked (ASCII msgs via `git commit -F`, no Co-Authored-By, quote bracket paths). `Bible-chat/` + `Ui-Design/` are gitignored.

**NEXT STEP:** **Today + Qur'an tabs are DONE + pushed.** Redesign the next P2 tab — **Ask (chat)** recommended next (the signature glass Ask pill + the AI flagship; it forces the reusable kit — GlassPill/MediaCard/Chip — that Explore + Community then reuse), then **Explore**, then **Community**. In onyx, device-review like Today. Reuse: the UI kit + theme tokens + the **reflect-style RN-`Modal` slide-up sheet** recipe + the **Listen-pill / headphone** audio language + `glass-surface.tsx`; preserve ALL existing logic (re-skin only).
