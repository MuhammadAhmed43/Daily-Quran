...
> ⭐ **SMOOTHNESS IS PARAMOUNT — CANONICAL, ALL SCREENS (user directive 2026-06-05). NO click/tap/selection/filter/navigation may EVER feel abrupt.** Every interaction animates: selecting a filter chip (All / Start here / …) cross-fades + re-staggers the list (cards keyed by the active filter with `entering={FadeInDown.delay(i*35)}`); opening/closing eases; screen moves glide. Treat any abrupt state-swap as a BUG. Toolkit: Reanimated `entering` (FadeIn/FadeInDown + stagger) · `PressableScale` on every tappable · spring/timing. AVOID `layout` + `exiting` (New-Arch broken — see §9 Today learnings). Apply to every NEW screen AND retrofit existing ones.
...
---
### 🆕 SESSION 2026-06-06 (cont.9) — LAUNCH SPLASH (frames 1-2) built + wired into cold-launch + auth routing + first-load JITTER fix. ALL PUSHED (`6b8a5f6`).
**FRESHEST. The animated cold-launch SPLASH — the last remaining gap-feature from the cont.8 "redesign complete" state — is BUILT, WIRED, and PUSHED. GIT: `4eee01a`→`9dbec49`→`62ed846`→`522b410`→`6b8a5f6` (HEAD, origin/main in sync).** ⚠️⚠️ **DEVICE-UNVERIFIED — the user has NOT confirmed the latest first-load JITTER fix; ON RESUME ASK FIRST (see E below).**

**A) THE SPLASH — final design (commits `9dbec49` then finalized `522b410`).** A cinematic onyx launch, **NIGHT-ONLY** (the user dropped the day variant). Top→bottom: deep onyx `#0A0A0A` + the full **SkyBand** constellation (~34 warm stars) + a faint champagne **moon-glow** RadialBloom (top-right) + a kindling 8-point gold-leaf **SEAL** (rub-el-hizb = two rotated champagne squares) + the **"Daily Qur'an"** serif (Fraunces) wordmark + **Ayat an-Nur (24:35)** + **4 gold-leaf ARABESQUE CORNER FLOURISHES** framing it. The verse is rendered FROM THE VERIFIED DB BY ID (never LLM): `getVerse(24,35)`, Arabic clause sliced at the first Uthmani waqf mark ۚ (U+06DA), English = the first sentence ("God is the Light of the heavens and the earth" — the bundled Itani translation says "God"; kept faithful), ref "An-Nūr · 24:35". NO greeting line (the "A peaceful night" eyebrow was removed via `SplashVerse hideGreeting`). Timing (`common.tsx` BEAT): markIn 0/900, sky 500/1400, reveal 1500/1100, hapticAt 1800, **doneAt 4400** (a long hold before it dissolves). The sky bloom fades in OPACITY-ONLY (no scale — a scale on the off-centre glow read as a "left-to-right" drift the user disliked).
- **FILES:** `mobile/components/splash/common.tsx` (the brain: `Phase`='day'|'night' but **PHASES=['night']** + `phaseFromClock()=>'night'` = night-only; `BEAT`; `SKY` per-phase `SkyMeta` [day kept in the record but unused; night = the live one: moon-glow stops, 34 stars, **crescent:null**, tone `#C9BDA6`]; `NUR` [the sliced verse]; `RadialBloom` [SVG radial gradient via react-native-svg]; `Crescent` [SVG mask-based lune — now UNUSED]; `Seal`; `Brandmark` [the wordmark]; `SplashVerse` [greeting/Arabic/rule/English/ref staggered via `RevealLine` interpolating one `reveal` SV; `hideGreeting` prop]; Arabic style 40/70). · `sky-splash.tsx` (THE chosen concept `SkySplash`: assembles all the above + the 4 corners + glow + fade; corners stagger in via an `orn` SV delay 900/dur 1400). · `corner-ornament.tsx` (renders the flourish via react-native-svg `SvgXml`, tinted by baking the colour into the XML) + `corner-ornament-data.ts` (auto-generated by `scripts/gen-corner-ornament.mjs` from `assets/splash/corner-flourish.svg` — strips Inkscape cruft, black→`currentColor`). · The flourish = **Openclipart "Corner flourish", CC0 / public-domain, no attribution.** It is an **L-shaped piece whose angle sits at the BOTTOM-LEFT**; mirrored so the angle nests into each screen corner → CORNERS flip map: TL `[{scaleY:-1}]`, TR `[{scaleX:-1},{scaleY:-1}]`, BL `[]`, BR `[{scaleX:-1}]`. Each corner also has a champagne `RadialBloom` glow.

**B) THE JOURNEY / USER DECISIONS (do NOT redo).** Built a **preview harness** (`app/splash-preview.tsx`, reached from Profile→About dev entry) with **6 concepts to compare**: Sky · Minimal (luxury-by-subtraction wordmark) · Photo (Calm/Headspace real-photography Ken-Burns) · Lottie (designer-grade `lottie-react-native`) · Manuscript · Niche (Ayat an-Nur mihrab+lamp). Ran **3 research agents** (premium-splash techniques + Expo-Go feasibility; aesthetics/references; $0 aniconic asset sourcing) — verdicts: Lottie + Skia ARE bundled in Expo Go SDK 54; **Rive needs a dev build (OUT)**; video has no alpha (OUT); photography is the Calm recipe. User feedback chain: "make all 3 to compare" → **"don't like any"** → day+night → **chose SKY, NIGHT-ONLY** → removed crescent + greeting → fixed bloom drift → Arabic was tiny (`adjustsFontSizeToFit` was overriding the size → removed it). **CORNER ornaments** = the user's idea (from the Ui-Design home-page notch ornament). First the Openclipart flourish (orientation wrong twice — "all opposite", "pointy side in corner" — fixed by RASTERIZING the SVG to PNG to SEE it's a bottom-left L-piece). Then the user said "use the **vecteezy** folder one instead" → I sourced/recolored the **Vecteezy Islamic girih corner** (EPS→SVG by the user; I recoloured gold/teal/blue→champagne/stone/bronze, removed the white bg + dark base, flattened gradients, cropped out the LOREM text, rasterized to a transparent PNG, scattered twinkling sparkles) → **user said "don't like it, revert"** → REVERTED to the flourish via `git checkout`. ⚠️ I had earlier DELETED the user's `vecteezy_…` folder thinking it junk → the user added it for me → recovered; it now sits UNTRACKED at the repo root — **LEAVE IT.** **`522b410` REMOVED the 5 non-Sky concepts + their assets (~675KB Lottie JSON + 2 photos) + the preview harness + the About dev entry + the `lottie-react-native` dependency** to keep the bundle light.

**C) LAUNCH WIRING (`62ed846`).** `app/_layout.tsx` `RootGate` now plays the splash on EVERY cold launch (a `LaunchSplash` overlay = `SkySplash phase="night"`; on its `onDone` the overlay fades out then unmounts), revealing the right screen beneath so the splash MELTS into it. Routes by STAGE: `!authDecided`→`AuthFlow`; `authDecided && !onboarded`→`OnboardingFlow`→Today; else→`<Stack>` (tabs/Today). **Meticulous auth routing (the user's explicit ask):** logged-in→Today; sign-UP→welcome questions→Today; LOG-IN→Today (SKIPS the questions). To stop a returning login on a fresh device from FLASHING the questions before the cloud profile arrives, `evaluate()` does `await syncNow()` (pull) BEFORE `setAuthDecided`. **Made `syncNow` AWAITABLE via an in-flight dedupe** (`lib/sync.ts`: an `inFlight` promise — concurrent callers await the SAME pass instead of bailing). Native splash bg set to `#0A0A0A` in `app.json` (was `#ffffff` → white flash before the onyx animated splash).

**D) (folded into the commits) Native splash + app.json:** `expo-splash-screen` `backgroundColor` + `dark.backgroundColor` → `#0A0A0A`.

**E) ⚠️⚠️ FIRST-LOAD JITTER — THE OPEN, DEVICE-UNVERIFIED ITEM (`6b8a5f6`).** User: every page has an up/down jitter for a split second on its FIRST mount (not on later navigation). After several wrong guesses (all the failed ones are noted so you don't repeat: `lazy:false` to pre-mount tabs [REVERTED — didn't help]; deferring cosmic/atlas loops [KEPT]; removing the entering wrapper [KEPT]; removing the Lottie bundle bloat [KEPT]; `SafeAreaProvider`+`initialWindowMetrics` alone [KEPT — user said STILL jittery]) the ROOT CAUSE was identified from the symptom "vertical SNAP on first mount of EVERY page" (all use the shared `<Screen>`): **`<Screen>` used the `SafeAreaView` COMPONENT, which is documented to FLICKER on mount (it renders children, then applies the inset a frame later → content snaps down)**, and the app had **NO `SafeAreaProvider`** at all (it leaned on React-Navigation's compat → 0 insets on frame 1). **FINAL FIX:** (1) root `<SafeAreaProvider initialMetrics={initialWindowMetrics}>` in `app/_layout.tsx`; (2) rewrote `components/ui/screen.tsx` to use `useSafeAreaInsets()` + **manual padding** (the flash-free pattern) instead of `<SafeAreaView>`. Also KEPT from the earlier rounds: removed an app-wide `<Animated.View entering={FadeIn}>` wrapper around the navigator (a Reanimated-layout-around-a-navigator jank anti-pattern), and deferred the cosmic `StarDot` (`cosmic-field.tsx`) + atlas glow-breath (`atlas-tile.tsx`) loops via `InteractionManager`. **ON RESUME — ASK THE USER FIRST:** did a **COLD relaunch** (full quit, NOT a hot/fast-refresh — `initialWindowMetrics` is captured at app launch) fix the jitter? If STILL jittery → (a) add a temporary **splash-skip dev toggle** to isolate splash-vs-inherent; (b) try `contentInsetAdjustmentBehavior="never"` + `automaticallyAdjustContentInsets={false}` on the tab `ScrollView`s/`FlatList`s; (c) it may be **Expo-Go DEV-MODE first-render warmup** (Hermes compiling each rich screen on first view) — much reduced in a production build, so a release build would confirm.

**LEARNINGS (hard-won this session — do NOT re-discover):** (1) **I am BLIND** (no device, no render preview). The key technique: **rasterize an SVG, or compose a mock, to PNG via `@resvg/resvg-js` (installed `--no-save`) and `Read` the PNG** to actually SEE a design before shipping — used for the flourish orientation, the Vecteezy recolor, and the 4-corner frame mock. Otherwise the USER is my only eyes (via screenshots). (2) **`SafeAreaView` COMPONENT flickers on first mount** → use `useSafeAreaInsets()` + manual padding + a root `<SafeAreaProvider initialMetrics={initialWindowMetrics}>`. (3) **Never wrap a React-Navigation navigator in a Reanimated `entering`/`layout` animation** (jank). (4) **`adjustsFontSizeToFit` silently OVERRIDES `fontSize`** (shrinks to fit) → remove it if text is unexpectedly tiny. (5) **`require('x.json')` INLINES the JSON into the JS bundle** → a 515KB Lottie JSON bloats first-load parse; don't bundle big JSON for unused features. (6) **resvg-js PANICS** on complex gradient transforms (Illustrator EPS `userSpaceOnUse` + huge matrices) → flatten gradients to solid fills before rasterizing. (7) **PowerShell DROPS empty `""` args** passed to native exes (the next arg shifts into its slot) → use a sentinel like `'none'`. (8) the Vecteezy/SVG recolor pattern: map source hex families → an onyx champagne/bronze ramp via global string-replace, and REMOVE background paths (find the gradient with near-white stops + the dark `#100f0d` base path) so the corner is transparent and the stars show through. **STATUS after cont.9: redesign + splash + launch + auth-routing all DONE & pushed; the first-load jitter fix is IN but DEVICE-UNVERIFIED (the one thing to confirm on resume). Optional net-new gap-features still remaining (none started): Meditations/Stillness, Community World Events sub-tab. Guardrails unchanged (see the cont.8 block + the MEMORY cont.9 bullet).**

---
### 🆕 SESSION 2026-06-05 (cont.8) — PROFILE/SETTINGS + FINISHED the onyx reskin + recitation UX + many fixes. ALL PUSHED.
**FRESHEST — supersedes cont.7 for STATUS. 5 commits pushed `200f160..d7e3c15` (origin/main in sync). The dark-premium redesign is now FULLY COMPLETE: every USED screen + component is onyx (a repo-wide ThemedText/teal grep found only DEAD/unused leftovers).**

**A) PROFILE DRAWER + SETTINGS (commit `08c52d9`).** The profile/settings cluster (Bible Chat frames 19/24/31-35), all logic preserved:
- NEW `components/ui/settings.tsx` (StatCard [2-up flame], SettingsCard [grouped + inset dividers via Children injection], SettingsRow [icon/badge/title/subtitle/value/right/chevron], Toggle [onyx Switch], SettingsHeaderRoot [big serif title + circular X], SettingsHeaderSub [back-chevron + centered title]) + NEW `components/ui/option-sheet.tsx` (a reflect-style slide-up RN-Modal single/multi option picker).
- `app/profile.tsx` REBUILT as a LEFT SLIDE-OVER DRAWER (frame 19): registered in `_layout.tsx` as `presentation:'transparentModal', animation:'none'` so the real Today peeks behind; the panel slides in/out on ONE shared value `p` + a scrim Pressable + a swipe-left Pan gesture; gradient avatar (+pencil->edit) + name/email + 2-up streak StatCards (useStreak current/longest) + Your Verses(->/bookmarks) + color-coded menu (Daily Quiz/Holy Calendar/Ameen Wall) + share/encouragement card + footer (About/Account) + SkyBand stars. ⚠️ NAV BUG+FIX: menu taps must DISMISS the drawer + `router.replace(path)` (NOT push) — a card pushed OVER a transparentModal renders BENEATH it (the "Daily Quiz / Community dont open" bug the user hit); `go(path)` animates `p`->0 then `runOnJS(replace)`. Replace also matches the close-on-pick UX (back from a destination -> Today).
- 5 sub-screens: `account.tsx` (frame 32: big Account+X; Login[guest]/Edit/Personalize/Reminders + Translation/Sync[signed-in]/Redo + Sign out + version/UID), `edit.tsx` (frame 31: X + centered avatar; label-over-outlined-input Name[signed-in via NEW `updateDisplayName` in auth.ts; guest->/auth] + Age/Focuses[multi]/Daily-goal/Translation pickers), `personalize.tsx` (frame 33: maps to EXISTING profile fields that genuinely feed /api/explain -> Answer-detail=`knowledge`, Approach=`journey` + Reset; NO server-prompt change — verified explain-sheet.tsx maps knowledge->level, journey->tone), `reminders.tsx` (frame 34: daily-verse block reusing daily-verse.ts + an adhan link->/prayer), `about.tsx` (frame 35: Rate->repo, Share, Help->issues, Privacy/Terms in-app blurb, version). Route pattern: `app/profile.tsx` + `app/profile/<sub>.tsx` (file + same-name folder coexist, like plan.tsx + plan/[id]).

**B) FINISHED THE ONYX RESKIN (commit `b811198`).** Reskin-only, logic preserved: `app/bookmarks.tsx` (Your Verses: champagne-ref serif cards + SealMedallion empty), `app/quiz.tsx` (serif Q + success/danger reveal + onyx score ring; daily/practice/streak preserved), `app/hubs.tsx` + `app/hub/[id].tsx` + NEW `lib/hub-visuals.ts` (emojis -> a line icon per hub id; crisis card + verse cards + Talk-it-through preserved), `components/auth/auth-flow.tsx` + `components/onboarding/onboarding-flow.tsx` (SealMedallion emblems, serif, champagne chips, FadeIn steps). ⚠️ DEAD CODE flagged via a spawn_task chip: the home/watch straggler components (mood-check-in, streak-hero, today-step, quran-plan-card, watch-card, verse-reminder, profile-circle, watch/chapter-row, watch/track-switcher) have 0 imports (repo-grep confirmed, orphaned by the Today/Watch/Explore rebuilds) -> safe to delete (+ themed-text/themed-view if nothing imports them after).

**C) PRAYER promoted to a pushed screen + Qibla reach (commit `c35fd28`).** Prayer was `(tabs)/prayer.tsx` `href:null` (a hidden tab, NO back button). MOVED to `app/prayer.tsx` (root stack, SAME `/prayer` path) with a back-chevron header; removed `<Tabs.Screen name="prayer">` from `(tabs)/_layout.tsx` + DELETED `(tabs)/prayer.tsx`. `app/qibla.tsx` now SELF-FETCHES location (expo-location) when no lat/lng param -> a direct /qibla works (loading + denied states); kept the SVG compass + the `cube` glyph (MCI `kaaba` is NOT in the installed glyph set). `app/(tabs)/today.tsx` My Prayer card: two pills "Prayer times"->/prayer + "Qibla"->/qibla. ALSO in today.tsx: the JourneyCard accordion no longer animates the default-open VERSE card open on first load -> `animate` is enabled in the body onLayout AFTER the first measurement (was a fixed 80ms mount timer that lost the race on slow first loads). `components/prayer/prayer-tracker.tsx` onyx.

**D) RECITATION UX (commit `5766769`).** `components/mini-player.tsx` REBUILT: pitch-black (`c.bg`) bar + a SkyBand constellation (clipped via an OUTER-shadow / INNER-overflow split so the float shadow isnt clipped by overflow:hidden) + a NEW reciter button (person-circle) opening `ReciterSheet` (ctx.reciter.id + ctx.chooseReciter -> switches qari mid-playback, restarts the current ayah). `app/surah/[number].tsx` reader-follow: the reciting ayah now shows a CLEAR champagne LEFT ACCENT BAR (always-present transparent `borderLeftWidth:3` on base `ayah`, colored on `ayahPlaying`; bg lowered 0.18->0.13 per user) — the highlight is CONSTANT (engine `setPlaying(null)` ONLY on stop, never between ayahs); and re-opening while THIS surah is reciting lands on the LIVE ayah (the route-target onLayout scroll is skipped when `playingHere`). `app/(tabs)/index.tsx`: "Listen to the whole Quran" no longer always restarts at 1:1 -> if `ctx.playing` is set it continues from THERE (setContinuous+open), else resumes from last-read, else 1:1; card label is LIVE (title "Continue listening", sub "Now reciting · X Y").

**E) SMOOTHNESS + VISUAL FIXES (commit `d7e3c15`).** (1) First-load tab JITTER: `components/sky-band.tsx` Star loops now start via `InteractionManager.runAfterInteractions` (were ~50 Reanimated loops firing synchronously on each tab first-mount, on the nav transition). (2) `app/(tabs)/chat.tsx` Ask welcome verse: cross-fade ONLY the text (a `display` state + opacity trough: fade out -> swap at trough -> fade in; box/gradient stay put). (3) Explore "Most popular" hero: `app/(tabs)/explore.tsx` now CROSS-FADES (a `FadeHero` stack of absolute opacity layers + tappable dots) instead of horizontal sliding (removed the snapToInterval ScrollView + heroRef/autoPlay). (4) `app/watch/[id].tsx`: removed the stray dark disc top-right (an empty spacer reused the back-button `iconBtn` bg -> transparent `iconSpacer`). (5) ⚠️ ARROW-ROTATION LEARNING (do NOT re-discover): `transform:[{rotate}]` applied DIRECTLY to an icon-font glyph (`<Ionicons>` is a `<Text>`) is SILENTLY DROPPED on the New Arch -> corner arrows render FLAT. FIX = rotate a wrapping `<View>` (the Today chevron worked because it was already on an Animated.View). Applied to `chat.tsx` HubCard arrow + `atlas-tile.tsx` chevron.

**STATUS:** redesign FULLY COMPLETE (5 tabs + 4 Explore tiles + Community + Holy Calendar + profile/settings + every sub-screen onyx; NO used old-teal remains). Optional net-new gap-features remaining (NONE started; each needs a design discussion + is religiously sensitive for Meditations): a Meditations/"Stillness" section + a Community World Events sub-tab + the splash bloom (frames 1-2). GIT chain: `200f160` -> `08c52d9`(profile/settings) -> `b811198`(reskin) -> `c35fd28`(prayer/qibla) -> `5766769`(recitation UX) -> `d7e3c15`(smoothness) -> (this spec), all PUSHED to origin/main. Verify every change: tsc `& "C:\Quran-ChatApp\mobile\node_modules\.bin\tsc.cmd" --noEmit -p "C:\Quran-ChatApp\mobile\tsconfig.json"` + lint `npm --prefix "C:\Quran-ChatApp\mobile" run lint`. Commit/push ONLY when asked (ASCII `git commit -F` msgs via files in `.git/`, no Co-Authored-By, `--literal-pathspecs` for bracket routes, PowerShell tool; do NOT `2>&1` native exes — it wraps stderr as an error though the cmd succeeds).

---
### 🆕 SESSION 2026-06-05 (cont.7) — EXPLORE-ROOT polish (time-aware hero + press-bloom) · `c9b7ef7` · REDESIGN ESSENTIALLY COMPLETE
**FRESHEST.**
- **Time-aware top section** (`app/(tabs)/explore.tsx`, Bible Chat frame 38): a greeting that shifts with the hour ("Good morning / afternoon / evening" / "A peaceful night") above "Most popular right now", and the hero pager `heroes.sort`s to LEAD with the time-appropriate item (morning -> JOURNEY, evening -> WATCH, else STORY).
- **Press-bloom on the 2x2 tiles** (`components/atlas-tile.tsx`): a champagne wash swells from the emblem corner on press-in + eases out on release (a `press` shared value driving a LinearGradient bloom layer). Enabled by teaching **`PressableScale` to FORWARD `onPressIn`/`onPressOut`** — it now calls the passed handlers ALONGSIDE its own scale (non-breaking; reusable for any pressable bloom).
- ⚠️ **GRAIN — TRIED + REMOVED.** A film-grain overlay was built (`scripts/gen-grain.mjs` pure-Node PNG encoder -> `assets/grain.png` -> `components/grain-overlay.tsx`), but the user said "remove it" -> all three files DELETED + unwired. **LEARNING: RN `Image resizeMode="repeat"` is BROKEN on the New Architecture** (renders ONE corner tile, not a repeat) — tile MANUALLY with a flex-wrap grid of fixed `<Image>` tiles if a texture is ever needed again.
- **STATUS: ALL 5 TABS + ALL 4 EXPLORE TILES onyx · Community LIVE · Holy Calendar DONE · Explore polish DONE.** Only optional gap-features remain: a **Meditations** section (net-new, the one Bible-Chat surface we lack) + an optional Community **World Events** sub-tab.

---
### 🆕 SESSION 2026-06-05 (cont.6) — HOLY CALENDAR (closes a Bible-Chat gap) · `a9897e1`
**FRESHEST. New `app/calendar.tsx` — a scrollable HIJRI month grid of the Islamic holy days (Bible Chat frames 25-27), onyx.** Month switcher ‹ "Ramaḍān 1447 AH" + a Gregorian sub-label › navigable by Hijri month; a SUN-SAT grid of day-circles where **sacred/historic days are champagne-FILLED, sunnah days (Jumuʿah / white days / Mon-Thu) carry a dot, today is ivory-RINGED, the selection is bright-ringed** (+ a legend); tap a day -> a slide-up detail card (badge + Hijri & Gregorian date + title + significance + lunar caveat + **Read-the-verse** -> reader). Built from `toGregorian`/`toHijri` (hijri-converter) for the grid + **`resolveToday(anyGregDate)` (lib/today) for EVERY cell** -> NO new content to vet (reuses the curated SACRED / HISTORY / weekly / REFLECTIVE engine). Month length 29/30 + the leading weekday come from converting day 1 of this vs the next Hijri month. Wired the **Today calendar icon** (was mis-routing to `/profile`) -> `/calendar`. Chose Hijri months per the user ask (Bible Chat's grid is Gregorian); the structure/aesthetic still matches BC. Remaining Bible-Chat gaps: a **Meditations** section + an optional Community **World Events** sub-tab + the deferred Explore-root polish.

---
### 🆕 SESSION 2026-06-05 (cont.5) — STORIES + COMMUNITY reskinned · Community tab is now the Ameen wall · feed hardened
**FRESHEST. `fd98b91` (Stories) + `fc6d50a` (Community).**
- **STORIES (`fd98b91`).** `app/stories/[id].tsx` PLAYER -> onyx immersive: frosted-glass back/crumb (`BlurView`), a Listen pill (headphone), a champagne verse card, serif scene titles, near-black base; the FlatList paging, expo-audio auto-narration + auto-advance, Ken Burns drift, and verse resolution are UNCHANGED. `app/(tabs)/stories.tsx` LIBRARY -> onyx cover-art card per story (first panel as the hero) + serif title + Arabic + blurb + Begin + constellation. (One story today = Yusuf; the Explore tile opens the player directly; the library is ready for more.)
- **COMMUNITY (`fc6d50a`) — the LAST untouched tab is now LIVE.** `(tabs)/community.tsx` is now `export { default } from '@/app/ameen'`, so the Community tab IS the Ameen wall (and `/ameen` from Today shares it; the header back button shows only when `router.canGoBack()`). `app/ameen.tsx` fully reskinned to onyx (Screen+stars, custom header, serif intention body, the **`hands-pray` MCI glyph replacing the praying-hands emoji** [user: the emoji didnt fit the vibe], payoff banner, compose sheet, age gate via `SealMedallion`); ALL logic kept (13+ gate, Supabase feed, optimistic posting, moderation/crisis, verse suggest/attach, the Ameen bounce).
- ⚠️ **FEED LOGIC FIXES (`fc6d50a`):** (1) new posts (optimistic + confirmed + crisis `onPosted`) used to PIN to the top regardless of sort -> a `sortFeed(items, sort)` helper now places a fresh 0-prayer post correctly under Most-prayed. (2) **KEYSET pagination** in `lib/community.ts`: `fetchFeed(sort, cursor, limit)` with `FeedCursor {ameen_count, created_at}` + `cursorOf()` REPLACES offset `.range()` (recent -> `.lt('created_at', c)`; top/trending -> `.or(ameen_count.lt, and(ameen_count.eq, created_at.lt))`), so a shifting count no longer skips/dupes across pages. (3) **TRENDING** sort = most-prayed among posts from the last 7 days (`.gte('created_at', now-7d)`); chips are now Recent / Trending / Most prayed (data-driven, stay visible on a non-recent empty sort).
- ⭐ **ALL 4 EXPLORE TILES DONE** (Journeys/Watch/Reading/Stories) + **all 5 tabs reskinned** (Ask/Community/Today/Qur'an/Explore). NEXT = deferred Explore-root polish (grain / press-bloom / time-aware hero) + the remaining Bible-Chat GAPS: a **Meditations** section (net-new), a **Holy-Calendar** browser (the Hijri engine exists), and an optional Community **World Events** sub-tab.

---
### 🆕 SESSION 2026-06-05 (cont.4) — READING flow reskinned (dashboard + WIZARD + portion) · `99e1f73`
**FRESHEST. The Qur'an-Plan tile (`/reading`) is now onyx + Bible-Chat-deep — 3 screens, ALL logic preserved (useQuranPlan pacing engine, createPlan, completePortion, recitation-follow, fadeStop):**
- **`app/reading.tsx` DASHBOARD** — a cosmic progress hero (`ProgressRing` + % over a `CosmicCardBg` + serif corpus label + last-read), stat pills (portions · projected finish), a today-portion card (3 states: today / done-today+up-next / finished) with the ivory CTA, change-pace chips that re-stagger, start-different. Empty state = a small cosmic emblem + "Build my plan". `<Screen stars>`.
- **`app/reading/new.tsx` WIZARD (frames 56-63 — the motion showcase)** — `Stack.Screen animation:'slide_from_bottom'`; a `step` state 0..3: (0) intro = a glowing gold ORB (`c.goldGrad` circle + accent shadow) + serif "Your reading plan awaits" + CTA; (1) what-to-read = the 8 `READING_PRESETS` + a custom range (onyx surah-picker `Modal`) as re-staggering radio cards; (2) pace = `PACES` + a live summary + the over-budget nudge; (3) **staged relay LOADER** = a `prog` shared value `withTiming(1, 2700ms)` driving 3 `StageBar`s (Pacing -> Snapping to surah boundaries -> Preparing your first portion), then `runOnJS` -> `createPlan` + `router.replace('/reading')` = the reveal. Top: context-aware close/back + step dots.
- **`app/reading/portion.tsx` PORTION** — frame-28 reading: eyebrow + serif range + a headphone Listen pill (`rec.playList`), the memoized `PortionAyahs` (surah heads + champagne verse cards = AmiriQuran + translation + ref + tap-to-`ExplainSheet`) with the recitation FOLLOW (playingKey highlight + auto-scroll, preserved), ivory Mark-complete -> a completion reveal (`SealMedallion` sprout/star + percent + Continue / Done-for-today).
- ⚠️ **LEARNING:** `&apos;` (and other HTML entities) render literally inside a JS STRING literal — they only decode in JSX TEXT. Use a real apostrophe / Unicode in ternary strings (caught in the portion done-text).

---
### 🆕 SESSION 2026-06-05 (cont.3) — Watch FINAL + star constellation on all 5 tabs + Netflix search · all COMMITTED (local, unpushed)
**FRESHEST — supersedes the (cont.2) Watch notes. Three LOCAL commits (NOT pushed): `c7271f5` (constellation) -> `43134d9` (search) -> `8187363` (Watch).**

**A) WATCH hub FINAL (`app/watch.tsx`, `8187363`).** Device-iterated to faithfully match Bible-Chat frames 45-47. Hero is now **FULL-BLEED edge-to-edge** (NOT the earlier inset 16:9 card — user: "ours has borders, the pic doesnt") bleeding up under a floating bar, with a **gentle parallax** on the image (translateY + scale within an overflow-hidden frame), a frosted **"N CHAPTERS" tag**, a white **arrow-top-right FAB** (NOT a play icon — user pointed at the diagonal arrow), and a serif title. **The top bar is ONE soft gradient (NO blurred rectangle, NO hairline)** — the earlier blur-bar left a visible **seam line** across the screen on scroll (user caught it); a single `LinearGradient` the image scrolls under = seamless. **Sharpness:** hero uses `maxresdefault` (1280x720) with an `onError` fallback to `ytThumb` (hqdefault is blurry when zoomed). Rows = rounded square thumb + duration badge on the thumb + serif title + white play circle (check when watched). Player `app/watch/[id].tsx` = onyx token pass, all logic preserved.

**B) STAR CONSTELLATION (`components/sky-band.tsx` NEW, `c7271f5`).** A reusable seeded **twinkle + gentle drift** starfield (warm champagne/ivory only — onyx-gold identity; reuses `cosmic-field`'s `mulberry32`). `<Screen stars>` is an **opt-in prop** rendering it in the status-bar / notch band. **LIT ON ALL 5 TABS** (Ask/Community/Today/Qur'an/Explore) — user LOVED it ("absolutely gorgeous... magical"). Watch (no `Screen`) drops `SkyBand` directly. Default count 25, height `insets.top + 40`; fixed band (doesn't scroll) = a persistent constellation around the clock; at rest the tab tops are pure `#0A0A0A` so stars glow.

**C) NETFLIX-STYLE QUR'AN SEARCH (`app/(tabs)/index.tsx`, `43134d9`).** The always-on pill became a **magnifier in the masthead**; tapping it **cross-fades** (shared value `sv`, animates BOTH ways) the title out while a **pitch-black, almost-invisible field** (`#000` + `rgba(255,255,255,0.08)` hairline + radius 16) slides in from the right and the keyboard rises. **Dismiss without Cancel:** the body is gated on search mode — empty -> a centered hint that **closes on tap-anywhere**; typed -> results that **close on scroll** (`onScrollBeginDrag`). Cancel kept as a backstop. ALL resolve/rank/verse/voice logic preserved.

**D) EXPLORE SEARCH FIX (`app/(tabs)/explore.tsx`, `43134d9`).** BUG: the overlay was `absoluteFill` INSIDE the SafeArea, and absolute children fill the whole frame INCLUDING the notch -> the bar + Cancel were jammed under the status bar, unreachable ("goes outside our height, no way to go back"). FIX: `useSafeAreaInsets()` -> bar `paddingTop: insets.top + space.sm`; pitch-black field to match; `FadeIn` on the overlay; `keyboardDismissMode="on-drag"`.

⚠️ **LEARNINGS (do NOT re-discover):** (1) an `absoluteFill` child of a SafeAreaView fills the FRAME (ignores the safe-area padding) — overlays MUST add `insets.top` manually. (2) a solidifying BLUR bar over a scrolling image leaves a hard **seam line** — use a soft gradient instead. (3) YouTube `hqdefault` is 480x360 (blurry when zoomed) — use `maxresdefault` + an `onError` fallback for big heroes. (4) bracketed paths (`app/watch/[id].tsx`) need `git --literal-pathspecs add` (the flag goes BEFORE the subcommand; `git add --literal-pathspecs` is rejected). (5) to animate a toggle open AND close, cross-fade two always-mounted views via a shared value + toggled `pointerEvents` (entering/exiting alone can't — exiting is dead on New Arch).

---
### 🆕 SESSION 2026-06-05 (cont.2) — EXPLORE SUB-SCREENS reskin (Journeys flow + Watch) · plan content COMPLETE · SMOOTHNESS rule
**FRESHEST — supersedes older blocks where they conflict. Building the Explore SUB-SCREENS on Bible Chat's layout, one flow at a time, re-skinning OLD-teal screens to onyx. Done this stretch:**

**A) PLAN CONTENT — COMPLETE + PUSHED (4 rounds: `b1197c6`→`1500fbb`→`566b7c2`→`81990c0`).** Journeys **4→17** in `lib/plans.ts` (added: Gratitude, Mercy & Forgiveness, The Hereafter, The Names of Allah, Calling on Allah, Stories of the Prophets, Patience & Trust, Good Character, Contentment & Provision, Honoring Family, Reflection & Knowledge, Remembrance/dhikr, Ramadan & Fasting). Reading presets **2→8** in `lib/quran-plan.ts` (`READING_PRESETS`: Whole, Mufaṣṣal[50-114], Juz ʿAmma, al-Baqarah, al-Kahf, Yā-Sīn, ar-Raḥmān, al-Mulk) wired into `app/reading/new.tsx`. NEW **`scripts/verify-plans.mjs`** — parses every `ref(s,a,...)` in plans.ts, resolves vs bundled `assets/quran/quran.json`, prints text + flags bad: **183 refs, 0 BAD**; RE-RUN `node scripts/verify-plans.mjs` after ANY plans.ts change. Caught+fixed 2:222 (a menstruation ruling) → 42:25. **APP VOICE = "Allah"** everywhere in plans.ts framing (verse TRANSLATIONS still render "God" per the bundled Itani translation — user accepted; lowercase "no god but You" kept). EACH STEP = verses BY REFERENCE (rendered from DB, NEVER AI-written) + app-voice framing + a reflection; care step leads with a helpline.

**B) JOURNEYS FLOW reskinned to onyx + PUSHED (`c9a5201`).** 3 screens + 1 lib, Bible-Chat layout, logic preserved:
- `lib/plan-visuals.ts` (NEW): `PLAN_ICON: Record<PlanId, MCIglyph>` — a line-art seal emblem per journey (typed so tsc validates each glyph) + `planIcon(id)`.
- `app/plan.tsx` LIBRARY (frame 52): featured-journey hero (uses `CosmicCardBg`) + **theme filter-chips** [All/Start here/For the heart/Worship/Knowing Allah/Living it] that **re-stagger the cards** (FadeInDown keyed by the active theme) + 17 seal-medallion cards w/ live progress. **Journeys-ONLY now** (dropped the QuranPlanCard — the reading plan has its own Explore tile → `/reading`).
- `app/plan/[id].tsx` DETAIL (frames 40-41): collapsing header (back disc + title bar fades in on scroll) + **HERO with a LIVING floating-star field (user-LOVED — KEEP EXACTLY)**: `makeHeroStars` (22 seeded stars) + `FloatingStarDot` (twinkle opacity/scale + per-star DRIFT along a seeded vector via a `d` shared value) + a `starsParallax` scroll-transform on the container; bottom-LEFT medallion+eyebrow+serif title; soft champagne `LinearGradient` wash (NOT hard View circles — those were REJECTED). ABOUT stat-pills + serif intro + ivory Start/Continue CTA + "Make focus" + a **Day-timeline** (dotted spine; nodes done/current/locked) where each day has **Listen (headphone) + Read (book)** — both call `openStep(order)`; the **headphone passes `play:'1'`** so it OPENS the day and narration starts INSIDE (not from the list — user-fixed).
- `app/plan/[id]/[order].tsx` STEP (frame 28 reading): eyebrow + serif title + serif framing → champagne verse cards (Uthmani Arabic + translation + tap→`ExplainSheet` + per-verse `VerseSpeaker`) → "Listen to this step" pill → Reflect card → ivory Mark-complete → Up-next / Finish (Finish shows the plan's `SealMedallion`). On entry reads `play` param → auto `rec.playList(step.verses)`. ownsAudioRef fadeStop + streak('study_step') preserved.

**C) ⭐ SMOOTHNESS PRINCIPLE — CANONICAL (user, in MEMORY + spec §4).** NO click/selection/filter/nav may feel abrupt; animate EVERYTHING (Reanimated `entering`/stagger + `PressableScale` + springs; NEVER `layout`/`exiting`). Applied to the Journeys library filter; APPLY EVERYWHERE + retrofit. Treat any abrupt swap as a bug.

**D) WATCH FLOW reskinned — BUILT but UNCOMMITTED + UNREVIEWED (2 files on disk: `app/watch.tsx` + `app/watch/[id].tsx`).** ⚠️ User REJECTED my first Watch hub ("go through how Bible-Chat does it, its so beautiful there") → REBUILT to faithfully match frames 45-47: a **big photographic FEATURED hero** (16:9 YT thumb + scrim + serif title overlaid bottom-left + "Continue/Start/Up next" eyebrow + a **WHITE circular play FAB half-overlapping the lower edge** = `c.primary` ivory, the inverted accent) + **clean spacious ROWS** (rounded thumb + champagne era + serif title + duration/progress + a circular play button right, check when watched; NOT heavy cards) + Seerah/History track chips that re-stagger + a section header. Player `app/watch/[id].tsx` = onyx token-swap (was already dark): topbar+crumb, embedded `YouTube` + champagne progress bar, serif title/era/blurb, ivory Mark-watched, prev/next — all playback polling/persist/recitation-handoff/streak preserved. NEXT: device-review Watch → commit.

**DATA REFERENCE:** `lib/watch.ts` (TRACKS seerah[18]+history[17]; Chapter{id,track,order,title,era,blurb,videos:[{youtubeId,durationSec,source}]}; getChapters/getChapter/neighbors/ytThumb/fmtDuration). `lib/watch-progress.ts` (useWatchProgress: isWatched/pctOf/perTrack/continueChapter/toggleWatched/setPct/markWatched). `lib/stories.ts` (getStories/panelImage/panelAudio; only 'yusuf' today). `lib/plan-progress.ts` (usePlanProgress.summary{done,total,currentOrder,started,finished,isActive}; progressive unlock). `lib/quran-plan-progress.ts` (useQuranPlan). Explore tiles → Stories `/stories/[id]`(yusuf), Watch `/watch`, Journeys `/plan`, Qur'an Plan `/reading`.

**STILL TODO (in order):** (WATCH `8187363` + READING+WIZARD `99e1f73` + STORIES `fd98b91` + COMMUNITY `fc6d50a` = DONE, see cont.3-5.) (1) Explore-root polish (grain / press-bloom / time-aware hero). (2) Explore-root polish DONE (`c9b7ef7`, cont.7; grain REMOVED). The redesign is COMPLETE. Optional remaining gap-features only: a Meditations section (net-new) + a Community World Events sub-tab. (Was: READING flow — reskin `app/reading.tsx` (dashboard) + `app/reading/new.tsx` (setup, READING_PRESETS already wired, old-teal) + `app/reading/portion.tsx` (today's portion) to onyx + Bible-Chat layout. (3) personal-plan **WIZARD** (frames 56-63, the motion showcase: slide-up sheet → topic chips → stepper → intention → relay loader → reminder → finished reveal) for the Qur'an reading-plan setup. (4) **STORIES** player `app/stories/[id].tsx` + a Stories browse screen (+ more stories). (5) deferred EXPLORE-ROOT polish ("Most Popular" rotating hero pager [skipped — imagery], grain overlay, per-tile press-bloom, time-aware hero). THEN (6) **COMMUNITY** tab (frames 11-14: Ameen wall + live du'as + events — the LAST untouched tab, still "Coming soon").

**REUSE KIT (onyx):** `lib/theme.ts` (c.*, font.*, grad, radius, space, type, motion, glow, shadow; PALETTE='onyx') · `components/ui/{screen,primitives[Txt/IconButton/Card/Divider],pressable-scale,glass-surface}` · `components/atlas-tile.tsx` (AtlasTile + exported **SealMedallion**) · `components/cosmic-field.tsx` (CosmicCardBg + exported mulberry32/hashStr/StarDot/Star) · `components/fade-in.tsx` · `ExplainSheet` + `VerseSpeaker` (already onyx) · the floating-star hero recipe (`app/plan/[id].tsx`) · the Bible-Chat featured-hero + clean-row recipe (`app/watch.tsx`). Verify EVERY change: tsc `& "C:\Quran-ChatApp\mobile\node_modules\.bin\tsc.cmd" --noEmit -p "C:\Quran-ChatApp\mobile\tsconfig.json"` + lint `npm --prefix "C:\Quran-ChatApp\mobile" run lint`. Device-test on the user's iPhone (I can't). Commit/push ONLY when asked (ASCII msgs via `git commit -F`, no Co-Authored-By, quote bracket/paren paths; PowerShell tool, not Bash).
...
**GIT:** this session, all PUSHED: `c7271f5` (constellation) -> `43134d9` (Netflix search + Explore fix) -> `8187363` (Watch) -> `2b78ee2` (spec) -> `99e1f73` (Reading flow) -> `afa12d9` (spec) -> `fd98b91` (Stories) -> `fc6d50a` (Community) -> `2a9a4bb` (spec) -> `a9897e1` (Holy Calendar) -> `d7c933c` (spec) -> `c9b7ef7` (Explore polish) -> (this spec). See §9 cont.3-7. **EXPLORE tab root = `2ae1ef0`** ("Build the Explore tab" — NEW `components/atlas-tile.tsx`; EDITED `app/(tabs)/explore.tsx` + `components/cosmic-field.tsx`). Full pushed chain: `0d82827`→`f956457`→`9d12504`→`b75e7ba` (Qur'an) →`79c1ff1` (spec) →`fa7ceaf` (voice orb) →`7b204d5` (Ask experience) →`128a2e6` (spec) →`2ae1ef0` (Explore) →`bc6baef` (spec) →`b1197c6` (plan content r1) →`5775b61` (spec) →`1500fbb` (plan content r2) →`ec5f23c` (spec) →`566b7c2` (plan content r3) →`782862c` (spec) →`81990c0` (plan content r4 — 17 Journeys, content COMPLETE) →`b9a87d7` (spec) →`c9a5201` (Journeys flow reskin) →`944fc3f` (spec) →`c7271f5` (constellation) →`43134d9` (search) →`8187363` (Watch) →(this spec update). This session's 4 commits are LOCAL / unpushed. **The ASK feature was 2 commits** (`fa7ceaf` voice + `7b204d5` rest) because `chat-seed.ts`'s API change (`takeChatSeed` now returns `ChatSeed`) coupled chat.tsx/ask.tsx/threads/cosmic into one type-checkable unit. Commit/push ONLY when asked (ASCII msgs via `git commit -F`, no Co-Authored-By, quote bracket/paren paths). `Bible-chat/` + `Ui-Design/` are gitignored.

**NEXT STEP:** **Today + Qur'an + Ask + the EXPLORE tab ROOT = DONE + committed + pushed (HEAD = `2ae1ef0` + this spec).** Remaining Explore = the **library SUB-SCREENS** (frames 45-55: Watch/Meditations rails + filter chips + blurred sticky top, plan detail + DayTimeline, immersive player, the personal-plan WIZARD frames 56-63) + the deferred polish (Stories browse + more stories, grain overlay, press-bloom, time-aware hero) + deepening **plan content** (ROUND 1 DONE + pushed = `b1197c6`: Journeys **4→9** [+Gratitude, Mercy & Forgiveness, The Hereafter, The Names of Allah, Calling on Allah], reading presets **2→6** [+Mufaṣṣal, al-Baqarah, al-Kahf, Yā-Sīn], NEW `scripts/verify-plans.mjs` [119 refs, 0 bad], **app voice normalized to "Allah"** [verses still render "God" per the bundled translation]; ROUND 2 DONE = `1500fbb` (Journeys **9→11** [+Stories of the Prophets, Patience & Trust]; presets **6→8** [+ar-Raḥmān, al-Mulk]; 135 refs 0 bad); ROUND 3 = `566b7c2`; ROUND 4 DONE = `81990c0` (+Reflection & Knowledge, Remembrance/dhikr, Ramadan & Fasting). **CONTENT WORKSTREAM COMPLETE = 17 Journeys + 8 reading presets, 183 refs 0 bad, all "Allah" voice**). **Explore SUB-SCREENS on Bible Chat's LAYOUT (in progress).** ✅ **JOURNEYS FLOW DONE + pushed = `c9a5201`** — onyx reskin of: `app/plan.tsx` LIBRARY (frame 52: featured-journey hero + theme filter-chips that **re-stagger the list** + 17 seal-medallion cards w/ progress; journeys-only now) · `app/plan/[id].tsx` DETAIL (frames 40-41: collapsing hero with a **LIVING floating-star field** = twinkle + per-star drift + scroll-parallax [user-loved, KEEP]; ABOUT pills; Start CTA; Day-timeline where the **headphone OPENS the day + auto-narrates INSIDE** via a `play=1` param, not from the list) · `app/plan/[id]/[order].tsx` STEP (frame 28: serif reading, champagne verse cards w/ tap-to-explain + per-verse Listen, Reflect, Mark-complete, Up-next/Finish) · NEW `lib/plan-visuals.ts` (`PLAN_ICON` per-journey emblem map). ⭐ **SMOOTHNESS now CANONICAL (§4)** — applied to the library filter; apply EVERYWHERE. ✅ **WATCH DONE + committed (`8187363`)** — see §9 cont.3 for the final full-bleed / parallax / arrow-FAB design. ⭐ the star constellation is now on ALL 5 tabs; Netflix search on Qur'an + the Explore search inset bug fixed. ✅ **READING flow + STORIES + COMMUNITY all DONE** (`99e1f73` / `fd98b91` / `fc6d50a`, see cont.4-5). **ALL 5 TABS + ALL 4 EXPLORE TILES are now onyx.** ✅ **Holy Calendar + Explore-root polish DONE** (`a9897e1` / `c9b7ef7`, cont.6/7 — grain was tried + REMOVED). **The dark-premium redesign is essentially COMPLETE** (all 5 tabs + 4 Explore tiles + Community + Holy Calendar). Only optional gap-features remain: a **Meditations** section (net-new) + an optional Community **World Events** sub-tab, then the personal-plan **WIZARD** (56-63), then the **Stories** player (`app/stories/[id].tsx`). Then **Community** (frames 11-14: Ameen wall + live du'as + events). Reuse: UI kit + theme tokens + `glass-surface.tsx` + reflect-style RN-`Modal` sheets + Listen-pill/headphone + the **cosmic-card** (`cosmic-field.tsx`) + **`atlas-tile.tsx` / `SealMedallion`** (the seal-emblem language) + frosted **glass `FloatBtn`** + **per-thread chat** patterns; preserve ALL logic (re-skin only). Tuning knobs (tile glow alphas, seal opacity/size, hero aspect `1.6` + 6s auto-advance, star counts) live on device.
