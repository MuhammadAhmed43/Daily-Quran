...
**GIT:** HEAD = `7b204d5` = `origin/main` (PUSHED, working tree CLEAN). The ENTIRE ASK FEATURE is now committed + pushed as **2 commits** atop `79c1ff1`: **`fa7ceaf`** (voice orb reskin — `app/voice.tsx`) + **`7b204d5`** (the Ask experience — 17 files: NEW `app/ask.tsx`, `app/chat-history.tsx`, `components/suggested-sheet.tsx`, `components/cosmic-field.tsx`, `lib/chat-history.ts`, `lib/welcome.ts`, `lib/suggested-questions.ts`; EDITED `app/(tabs)/chat.tsx`, `app/(tabs)/today.tsx`, `app/_layout.tsx`, `app/hub/[id].tsx`, `app/reflect.tsx`, `components/explain-sheet.tsx`, `components/speak-button.tsx`, `lib/chat-seed.ts`, `package-lock.json`, this spec). **Split is 2 not 4** because `chat-seed.ts`'s API change (`takeChatSeed` now returns `ChatSeed`, not `string`) couples chat.tsx/ask.tsx/threads/cosmic into one type-checkable unit; voice was the only cleanly separable piece. Earlier pushed chain: `0d82827`→`f956457`→`9d12504`→`b75e7ba` (Qur'an) →`79c1ff1` (spec) →`fa7ceaf`→`7b204d5`. Commit/push ONLY when asked (ASCII msgs via `git commit -F`, no Co-Authored-By, quote bracket paths). `Bible-chat/` + `Ui-Design/` are gitignored.

**NEXT STEP:** **Today + Qur'an + the WHOLE ASK FEATURE = DONE + committed + pushed (HEAD `7b204d5`).** Next tab — **Explore** (frames 37-51: rotating hero + 2x2 hub + continue carousel + recommended rails + search), then **Community** (frames 11-14: Ameen wall + live du'as + events). Reuse: UI kit + theme tokens + `glass-surface.tsx` + reflect-style RN-`Modal` sheets + Listen-pill/headphone + the **cosmic-card** (`cosmic-field.tsx`) + frosted **glass `FloatBtn`** + **per-thread chat** patterns; preserve ALL logic (re-skin only). Cosmic tuning knobs (glow alphas in `GLOWS`, star count/brightness in `makeStars`) live on device.
