...
**FRESHEST STATE — where this conflicts with the older "ASK TAB" block below, THIS wins. Everything here is BUILT + tsc/lint-clean + heavily device-tested-and-iterated this session, but STILL 100% UNCOMMITTED (HEAD `79c1ff1`). It all sits in the working tree as one big "Ask feature" awaiting a commit.**
...
**GIT:** HEAD = `79c1ff1` (a spec-update commit; the last PUSHED feature commit is `b75e7ba` Qur'an tab — pushed chain `0d82827`→`f956457`→`9d12504`→`b75e7ba`; `79c1ff1` may be a local-only spec commit). ⚠️ **The ENTIRE ASK FEATURE (Ask conversation + browse + welcome opener + voice orb + cosmic cards + per-category threads) is UNCOMMITTED in the working tree** — NEW files: `app/ask.tsx`, `app/chat-history.tsx`, `components/suggested-sheet.tsx`, `components/cosmic-field.tsx`, `lib/chat-history.ts`, `lib/welcome.ts`, `lib/suggested-questions.ts`; EDITED: `app/(tabs)/chat.tsx`, `app/(tabs)/today.tsx`, `app/_layout.tsx`, `app/hub/[id].tsx`, `app/reflect.tsx`, `app/voice.tsx`, `components/explain-sheet.tsx`, `components/speak-button.tsx`, `lib/chat-seed.ts`, `package-lock.json`, this spec. tsc+lint clean, heavily device-tested + iterated this session. Commit/push ONLY when asked (ASCII msgs via `git commit -F`, no Co-Authored-By, quote bracket paths). `Bible-chat/` + `Ui-Design/` are gitignored.

**NEXT STEP:** **Today + Qur'an = DONE + pushed. The whole ASK FEATURE = built + heavily device-tested this session but UNCOMMITTED.** (1) **COMMIT + PUSH the entire Ask feature** once the user is happy (likely split into logical commits: Ask conversation/browse, per-category threads, voice orb, cosmic cards). (2) THEN the next tab — **Explore** (frames 37-51: rotating hero + 2x2 hub + continue carousel + recommended rails + search), then **Community** (frames 11-14: Ameen wall + live du'as + events). Reuse: UI kit + theme tokens + `glass-surface.tsx` + reflect-style RN-`Modal` sheets + Listen-pill/headphone + the **cosmic-card** (`cosmic-field.tsx`) + frosted **glass `FloatBtn`** + **per-thread chat** patterns; preserve ALL logic (re-skin only). Cosmic tuning knobs (glow alphas in `GLOWS`, star count/brightness in `makeStars`) live on device.
